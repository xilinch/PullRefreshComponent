 ...
 * 与传统的上下拉操作刚好相反
 */
abstract public class RefreshReverseLayout extends FrameLayout {
    /**
     * 上下拉效果的控件
     */
    ...
    public RefreshReverseLayout(Context context) {
        super(context);
    }

    public RefreshReverseLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initLayout(context, attrs);
    }

    public RefreshReverseLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        loadingLayout.setVisibility(View.GONE);
        ...
        mPtrRefreshLayout = (PtrNFClassicFrameLayout) findViewById(R.id.ptrRefreshLayout);
    ...
    private void initZeroDataLayout() {
        ...
        dataZeroTextView = (TextView) dataZeroLayout.findViewById(R.id.dataZeroTextView);
        dataZeroTextView2 = (TextView) dataZeroLayout.findViewById(R.id.dataZeroTextView2);
        dataZeroButton = (Button) dataZeroLayout.findViewById(R.id.dataZeroButton);
        emptyTopView = (LinearLayout) dataZeroLayout.findViewById(R.id.empty_top_view);
    }
...
//                    checkAndLoad();
    ...
    public void checkAndLoad() {
        try {
            if (!isRequesting) { // 必须有多页 && 必须是之前的请求已返回后
...
//        }
    ...
    public void showLoadingView() {
        ...
        loadingTextview.setText(getResources().getString(R.string.pull_newslist_more_loading_text));
    ...
    protected synchronized void updateList(boolean append, List list, PlusRecyclerAdapter adapter, boolean isAddAtBegin) {
        if (list == null) {
            list = new ArrayList();
        }
    ...
    public void updateAdd(List list) {
        updateList(true, list, adapter, false);
    }

    public void updateAddAtBegin(List list) {
        updateList(true, list, adapter, true);
    }

    public void updateClearAndAdd(List list) {
        updateList(false, list, adapter, false);
    }
